---
clase: 11
titulo: Impuestos — Efectos económicos y pérdida de eficiencia
unidad: Unidad II
fecha: Miércoles 8 de abril
---

::: hero
::eyebrow
CLASE 11 · UNIDAD II · SEMANA 6 — MIÉRCOLES

::titulo
Impuestos — Efectos económicos y pérdida de eficiencia

::subtitulo
Case & Fair Cap. 4 (pp. 89–93), Cap. 19 (pp. 397–404) · Hazlitt (lectura asignada)

::meta
DERE-A0004 · Introducción a la Economía · Universidad Autónoma de Chile
:::

::: roadmap
::kicker
ESTRUCTURA DE LA CLASE

::titulo
Cinco bloques para entender el impuesto.

::p1-titulo
Repaso y conexión
::p1-body
Desde precios máximos/mínimos hacia los impuestos.

::p2-titulo
Excedente del consumidor y del productor
::p2-body
Concepto nuevo, construcción gráfica en pizarra.

::p3-titulo
La cuña impositiva
::p3-body
Diagrama, distribución del bienestar, peso muerto.

::p4-titulo
Incidencia tributaria
::p4-body
Quién paga y cuánto, el rol de la elasticidad.

::p5-titulo
Cierre y tarea
::p5-body
Verificación, ejercicios Canvas, anticipación Clase 12.
:::

::: manifesto
::titulo
Repaso — ¿Qué vimos la semana pasada?

::q1
¿Qué condición hace que un precio máximo genere escasez?

::q2
¿Qué mecanismos alternativos emergen cuando hay precio máximo y el precio no puede subir?

::q3
¿Por qué el salario mínimo puede generar desempleo según el modelo?

::footer
Tres preguntas para activar la clase anterior antes de entrar al impuesto.
:::

::: intro
::kicker
BLOQUE 1 · CONEXIÓN

::titulo
Desde los precios controlados hacia el impuesto.

::body
La semana pasada vimos dos formas de intervención directa:

- **Precio máximo.** Se fija por debajo del equilibrio → genera escasez permanente.
- **Precio mínimo.** Se fija por encima del equilibrio → genera superávit permanente.

Hoy: el impuesto. En lugar de fijar el precio, el gobierno cobra un monto adicional por cada unidad comprada o vendida. La pregunta es: ¿quién absorbe ese costo? ¿Qué se pierde en el camino?
:::

::: grid-fallas
::titulo
Bloque 2 — los tres conceptos del bienestar

::f1-titulo
Excedente del consumidor (EC)
::f1-body
Lo que el comprador 'ahorra': la diferencia entre lo que estaba dispuesto a pagar y lo que efectivamente paga. Es el área bajo la curva de demanda y por encima del precio de mercado.

::f2-titulo
Excedente del productor (EP)
::f2-body
La ganancia del vendedor: la diferencia entre el precio que recibe y lo que le cuesta producir. Es el área sobre la curva de oferta y por debajo del precio de mercado.

::f3-titulo
Bienestar social total
::f3-body
La suma EC + EP mide el valor total que genera el mercado para compradores y vendedores. En competencia perfecta, el equilibrio maximiza ese total.

::f4-titulo
Áreas en equilibrio (Q*)
::f4-body
**EC** = triángulo arriba de P*. **EP** = triángulo abajo de P*. **EC + EP** es el bienestar máximo posible. Cualquier cantidad distinta a Q* lo reduce. Esto es la eficiencia del mercado competitivo.
:::

::: intro
::kicker
BLOQUE 3 · LA CUÑA IMPOSITIVA

::titulo
El impuesto como cuña entre comprador y vendedor.

::body
La clave es la ley de la demanda: el impuesto sube el precio que paga el comprador → a mayor precio, menor cantidad demandada → algunas transacciones dejan de ocurrir → ahí surge la pérdida de eficiencia.

Cuando el gobierno cobra un impuesto por cada unidad vendida, se crea una brecha entre el precio que paga el comprador y el precio que recibe el vendedor. A esa brecha la llamamos **cuña impositiva**.
:::

::: mecanismo
::titulo
Los tres efectos de la cuña

::subtitulo
Cuando el gobierno aplica un impuesto por unidad vendida

::paso1
**El comprador paga más**
El precio que enfrenta el comprador sube por encima del precio de equilibrio. Parte del costo del impuesto se traslada al consumidor.

::paso2
**El vendedor recibe menos**
El precio neto que recibe el vendedor cae por debajo del precio de equilibrio. Parte de la carga la absorbe el productor.

::paso3
**La cantidad transada cae**
La brecha hace que transacciones que habrían ocurrido en equilibrio ya no se realicen. Ahí surge la pérdida de eficiencia.
:::

::: grid-fallas
::titulo
¿A dónde va el bienestar con el impuesto?

::f1-titulo
Excedente del consumidor — se reduce
::f1-body
Los compradores ahora pagan más que el precio de equilibrio. Su excedente cae.

::f2-titulo
Recaudación fiscal — va al gobierno
::f2-body
Parte del bienestar que antes era de compradores y vendedores pasa al gobierno. **No desaparece**: el gobierno lo recibe y puede usarlo para financiar gasto público.

::f3-titulo
Excedente del productor — se reduce
::f3-body
Los vendedores ahora reciben menos que el precio de equilibrio. Su excedente cae.

::f4-titulo
Peso muerto — no va a nadie
::f4-body
Una parte del bienestar se **destruye irreversiblemente**. Son las transacciones que habrían beneficiado a compradores y vendedores pero que ya no ocurren por efecto del impuesto.
:::

::: intro
::kicker
BLOQUE 3 · DEFINICIÓN CLAVE

::titulo
El peso muerto — bienestar que no va a nadie.

::body
**Peso muerto (deadweight loss).** Pérdida neta de bienestar que no captura ningún agente — ni consumidores, ni productores, ni el gobierno. Es la destrucción irreversible de valor que ocurre porque el impuesto hace que transacciones mutuamente beneficiosas dejen de ocurrir.

*"Los efectos visibles de un impuesto son los ingresos que recauda el gobierno. Los efectos invisibles son las cosas que no se produjeron y los intercambios que no ocurrieron."* — **Henry Hazlitt**, *Economía en una lección* (lectura asignada).
:::

::: stat-split
::kicker
PÉRDIDA DE EFICIENCIA (FIG. 4.9 — CASE & FAIR, CAP. 4, P. 89)

::leftlabel
a) SUBPRODUCCIÓN — EL CASO DEL IMPUESTO
::leftnum
ABC
::leftpct
Triángulo = peso muerto
::leftformula
Unidades 4 → 7 no se transan

::rightlabel
b) SOBREPRODUCCIÓN — MAÑANA: EL SUBSIDIO
::rightnum
ABC
::rightpct
Triángulo = peso muerto
::rightformula
Unidades 7 → 10 cuestan más de lo que valen

::insight
**a) Subproducción.** El impuesto fuerza al mercado a operar con menos unidades que el equilibrio. Las unidades entre 4 y 7 no se transan, y el bienestar que habrían generado desaparece. Ese triángulo es el **peso muerto**. **b) Sobreproducción.** El subsidio fuerza a producir más de 7 millones. Las unidades entre 7 y 10 cuestan más producirlas de lo que los consumidores las valoran. Ese exceso también es pérdida neta.
:::

::: intro
::kicker
BLOQUE 4 · INCIDENCIA TRIBUTARIA

::titulo
La incidencia no depende de a quién cobra el gobierno.

::body
El gobierno puede cobrar el impuesto formalmente al vendedor o al comprador. Eso **no** determina quién soporta la carga real. La distribución depende de las **elasticidades**.
:::

::: grid-fallas
::titulo
Incidencia — dos casos extremos, regla general y aplicación

::f1-titulo
Demanda muy inelástica
::f1-body
Compradores no pueden sustituir fácilmente. Los compradores absorben la mayor parte del impuesto. El precio del comprador sube casi tanto como el impuesto. El precio del vendedor cae poco. *Ej.: medicamentos de urgencia, gasolina en el corto plazo.*

::f2-titulo
Oferta muy inelástica
::f2-body
Productores no pueden reducir cantidad fácilmente. Los productores absorben la mayor parte del impuesto. El precio del vendedor cae casi tanto como el impuesto. El precio del comprador sube poco. *Ej.: tierras agrícolas (la oferta de tierra es fija).*

::f3-titulo
Regla general
::f3-body
Quien tiene menos capacidad de ajustar su comportamiento ante el cambio de precio soporta la mayor parte de la carga impositiva.

::f4-titulo
Aplicación al IVA chileno (19%)
::f4-body
Si el gobierno le cobra el IVA al vendedor, ¿significa que el vendedor paga todo? **No.** El vendedor traslada parte al consumidor subiendo el precio. La proporción que puede trasladar depende de cuánto puede ajustar el consumidor su comportamiento — es decir, de la **elasticidad de la demanda**. **Pregunta al curso.** Si el gobierno quiere recaudar con el menor peso muerto posible, ¿qué tipo de bienes debería gravar?
:::

::: exercise-intro
::kicker
EJERCICIO EN PAREJAS O TRÍOS · 8 MINUTOS

::titulo
El impuesto al tabaco en Chile

::datos
Pe = $1.500 · Q antes = 10M cajetillas/mes · Impuesto = $500 · Pc = $1.800 · Pv = $1.300 · Q después = 7M cajetillas/mes

::nota
El gobierno aplica un impuesto de $500 por cajetilla de cigarrillos. Discutan en parejas o tríos las cuatro preguntas siguientes.
:::

::: intro
::kicker
EJERCICIO · LAS CUATRO PREGUNTAS

::titulo
Cuatro preguntas para discutir.

::body
**1.** ¿Cuánto es la cuña impositiva? ¿Coincide con el monto del impuesto?

**2.** ¿Quién soporta mayor parte de la carga: compradores o vendedores? ¿Se comparte la carga? ¿Qué nos dice eso sobre la elasticidad de la demanda?

**3.** La cantidad cae de 10 a 7 millones. ¿Hay pérdida de eficiencia? ¿Por qué? ¿A quién le va esa pérdida?

**4.** ¿Es este un buen impuesto desde el punto de vista del bienestar social? ¿Y desde el punto de vista del gobierno?
:::

::: intro
::kicker
EJERCICIO · REVISIÓN — PREGUNTAS 1 Y 2

::titulo
Cuña impositiva e incidencia.

::body
**1.** La cuña es **$500** ($1.800 − $1.300) — coincide con el monto del impuesto. Siempre es así. A quién se le cobra formalmente no importa: la incidencia la determina la elasticidad, no la ley.

**2.** Compradores absorben **$300** ($1.800 − $1.500) y vendedores **$200** ($1.500 − $1.300). La carga se comparte: **60% compradores, 40% vendedores**. Que ambos soporten parte indica que la demanda no es perfectamente inelástica — pero los compradores sienten más el impuesto.
:::

::: intro
::kicker
EJERCICIO · REVISIÓN — PREGUNTAS 3 Y 4

::titulo
Peso muerto y evaluación de la política.

::body
**3.** Sí. Las **3 millones** de cajetillas que dejaron de venderse habrían generado bienestar para ambas partes. Ese bienestar no va a nadie — es el **peso muerto**. No lo captura ni el gobierno.

**4.** Debate abierto. Desde la eficiencia: hay peso muerto. Desde la recaudación: funciona porque la demanda es inelástica. Desde la salud pública: la distorsión puede ser el **objetivo** — reducir el consumo de tabaco. Los tres criterios coexisten.
:::

::: diagrama
::kicker
EJERCICIO TABACO — VISUALIZACIÓN

::d1-tipo
cuna-tabaco

::d1-titulo
La cuña del impuesto al tabaco

::d1-leyenda
**D** = demanda · **O** = oferta · **$1.500** = Pe (equilibrio sin impuesto) · **$1.800** = Pc (precio consumidor) · **$1.300** = Pv (precio vendedor) · **7M** = Qt (cantidad con impuesto) · **10M** = Qe (cantidad sin impuesto) · rectángulo = **recaudación** · triángulo = **peso muerto**

::d1-texto
El impuesto de $500 abre la cuña Pc−Pv. El rectángulo es lo que recauda el Estado. El triángulo son las 3 millones de cajetillas que dejaron de transarse — bienestar destruido que no va a nadie. Los consumidores absorben el **60%** de la carga ($300); los vendedores el **40%** ($200).
:::

::: manifesto
::titulo
Verificación rápida antes de cerrar.

::q1
¿Qué es el peso muerto de un impuesto? ¿A quién le va ese dinero?

::q2
¿Qué significa que el IVA sea neutral respecto de a quién se le cobra formalmente?

::q3
¿Qué relación hay entre la elasticidad de la demanda y el tamaño del peso muerto?

::footer
Si alguna de las tres incomoda, repasen el bloque correspondiente antes del jueves.
:::

::: close
::eyebrow
TAREA · JUEVES

::titulo
Mañana: el subsidio — el espejo del impuesto.

::body
**Clase 12 — Subsidios y análisis de la intervención tributaria.** Cuña del subsidio, efectos sobre el bienestar. Comparación impuesto vs. subsidio. Tipos de impuesto: progresivo, proporcional, regresivo.

::reglas
excedente del consumidor · excedente del productor · cuña impositiva · incidencia · elasticidad · peso muerto

::meta
DERE-A0004 · Introducción a la Economía · Universidad Autónoma de Chile
:::
